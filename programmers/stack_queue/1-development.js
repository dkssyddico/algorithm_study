...
function otherSolution(progresses, speeds) {
  ...
  for (let i = 0, j = 0; i < days.length; i++) {
    if (days[i] <= maxDay) {
      console.log('smaller', answer);
      answer[j] += 1;
    } else {
      console.log('bigger', answer);
      maxDay = days[i];
      answer[++j] = 1;
    }
  }
...
let p8 = [40, 93, 30, 55, 60, 65];
let s8 = [60, 1, 30, 5, 10, 7];

let p7 = [93, 30, 55, 60, 40, 65];
let s7 = [1, 30, 5, 10, 60, 7];

// console.log(otherSolution(p7, s7));
console.log(otherSolution(p8, s8));
