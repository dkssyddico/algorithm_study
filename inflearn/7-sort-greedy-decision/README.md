...
그리디 알고리즘은 경우의 수가 존재할 경우,
경우를 선택해야할 때 최선이라고 생각하는 경우를 고르는 알고리즘이다.
전체를 고려하지 않고 그 순간에 최선, 그때그때 가장 좋아보이는 것을 고르는 것이다.
예) 지금 새우깡을 먹거나, 10분 참으면 다금바리를 먹을 수 있는 경우
➡️ 🦐 새우깡을 먹는다
그리디 알고리즘이 내놓은 값은 항상 최적화된 값이 아닐 수도 있다.
그리디 알고리즘은 기본적으로 무조건 큰 경우대로, 무조건 작은 경우대로, 무조건 긴 경우대로, 무조건 짧은 경우대로 등 극단적으로 문제에 접근한다는 점에서 sort 기법이 함께 사용된다.
예) 500원, 100원, 10원, 1원이 있을 때 70원에 대한 거스름돈 거슬러주기.
➡️ 500원으로 먼저 해보고, 100원으로, 10원으로 1원으로
...
배열을 돌면서 도착한 시간, 나가는 시간을 구한다.
그 다음 사람이 도착한 시간을 구해, 기준 사람의 도착한 시간과 나가는 시간 사이에 있는지 비교한다.

### 👨‍🏫 Teacher's solution

각 사람마다 온 시간, 간 시간을 두 개의 이벤트로 쪼갠다.
ex) [14, 's'], [18, 'e']
왔을 때는 사람 수를 카운트하고, 나갈 때는 사람 수를 마이너스한다.
같은 시간대면 e가 먼저와야 한다.
[5, 's'], [12, 's'], [14, 'e'], [14, 's'] .. 이런식으로
안그러면 가는 시간과 오는 시간이 같은 값일 때 카운트가 먼저 되버린다.
오는 시간을 먼저 빼주는 게 중요함.
cnt라는 변수를 만들고, for문으로 s 인지 e 인지 비교하고 카운트 해주고 answer에 가장 큰 수를 저장한다.
...
이분 검색에서는 left, right를 가리키는 포인트 변수가 필요하다.
이분 검색은 무조건 정렬이 되어있어야 함
mid 변수. lt와 rt를 더해서 2를 나눈다
그리고 mid가 target인지 확인.
아니라면 lt나 rt의 값을 바꾼다
만약 찾은 값이 타겟보다 크면 rt의 값을 바꾼다.
rt는 mid에서 하나 뺀 값으로.
lt는 mid에서 하나를 더한다.
while문을 돌려 확인하되 조건은 lt가 rt보다 크거나 작거나 이다.
mid값과 target이 일치하지 않을경우 lt는 점점 커지고 rt는 점점 작아지기 때문이다.
...
이분 검색을 이용.
lt는 시간에서 가장 큰 수(9), rt는 모든 시간을 합한 수(45)가 된다.
둘을 더해 2로 나눈 mid(27)는 다 포함할 수 있다. 27 위의 수는 볼 필요가 없다.
이 mid가 arr안에 숫자 합을 판별할 수 있는 함수를 만든다.
sum을 0으로 하고, 배열 내 아이템을 차례로 sum에 더한다.
만약에 mid보다 숫자합이 작으면 계속 더하고, 숫자합이 크면 mid만큼의 합이 만들어졌다는 뜻이므로 카운트를 하나 올린다.
그리고 카운트를 return한다.
만약 이 카운트가 주어진 m개보다 작다면 아직 더 작은 수를 구할 수 있다는 뜻이므로,
이분 검색처럼 rt를 mid 보다 하나 작은걸로 만들어준다.
만약에 카운트가 주어진 m보다 크면 그건 mid 용량이 작은 것을 의미해서 lt 값을 올려주어야한다.
