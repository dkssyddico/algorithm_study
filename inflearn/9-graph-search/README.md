...
영역을 확인하는 것은 BFS로(queue) 사용으로도 풀 수 있다.
맨처음 이중 for문을 이용해 board[0][0]부터 섬을 찾기 시작한다.
섬을 만나면(1이면), 바로 바다인 0으로 만들어주고 큐에 해당 좌표를 넣어준다.
그리고 while문을 실행한다.
while문은 큐에 아무것도 없을 때까지 실행한다.
처음으로는 큐에 넣어진 아이를 맨 앞에서부터 뺀다.
그리고 저번처럼 8 방향을 검사해서 새로운 섬이 있는지 찾는다.
새로운 섬을 찾는 경우 그 좌표를 다시 큐에 넣어주고 위의 과정을 반복한다.
